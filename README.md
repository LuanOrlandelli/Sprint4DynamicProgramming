...
🧠 Gestão de Consumo nas Unidades de Diagnóstico — Programação Dinâmica
...
Registrar o consumo diário de materiais (Fila).

Visualizar o consumo em ordem cronológica (FIFO) ou inversa (Pilha/LIFO).

Buscar produtos por nome usando busca sequencial e binária.

Ordenar produtos por consumo total com Merge Sort e Quick Sort.

Aplicar Programação Dinâmica (DP) para prever a demanda e determinar uma política de pedidos que minimize custos operacionais.
...
💰 Função Objetivo
custo_total = (K se q>0) + c*q + h*s' + p*shortage
